...
/*hàm đánh giá leadingOnes*/
int leadingOnes(bool* v)
{
    int count = 0;
    for (int i = 0; i < problem_size; i++)
    {
...
int leadingOnesFitness(individual temp)
{
    number_of_evaluations++;
    return leadingOnes(temp.value);
}
	...
	for (int i = 0; i < n; i++)
	{
		...
		case 1:
			*population[i].fitness = trap5Fitness(population[i]);
			break;
		case 2:
			*population[i].fitness = leadingOnesFitness(population[i]);
			break;
		default:
			break;
		}
	...
	for (int i = 0; i < n; i += 2)
	{
		...
		case 1:
			...
			*offspring[i + 1].fitness = trap5Fitness(offspring[i + 1]);
			break;
		case 2:
			*offspring[i].fitness = leadingOnesFitness(offspring[i]);
			*offspring[i + 1].fitness = leadingOnesFitness(offspring[i + 1]);
			break;
		default:
			break;
		}
...
bool check(int n)
{
	int MSSV = 21521749 /*MSSV*/;
	random_seed = MSSV + k_bisection * 10;
	for (int i = 0; i < 10; i++)
	{
		if (!GA(n, random_seed + i))
			return false;
	}
	return true;
}
...
int main()
{
	...
	cin >> problem_size;
	cout << "Chon function Onemax hay trap5 hay LeadingOnes: (0: onemax, 1: trap5, 2: LeadingOnes): ";
	cin >> problem_type;
	...
	cout << "Ket qua MRPS cua GA cho phep lai va function tuong ung la:" << mean_of_MRPS / (10.0) << ' ' << stdDeviation(MRPS, mean_of_MRPS / 10.0) << endl;
